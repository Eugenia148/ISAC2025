...
# Genera métricas defensivas, radar charts y etiquetas por posición
# ==========================================
import os
import ast
import math
...
# ==========================================
# GENERAR DESCRIPCIÓN CUALITATIVA DE CLÚSTER
# ==========================================
def describe_cluster(row):
    """Etiqueta automática de perfil según promedio de métricas."""
    desc = ""
    if row["tackles_90"] > row["interceptions_90"] and row["pressures_90"] > row["clearance_90"]:
        desc = "Defensa agresivo en duelos"
    elif row["aerial_ratio"] > 0.6 and row["clearance_90"] > row["tackles_90"]:
        desc = "Central dominante por aire"
    elif row["forward_pass_proportion"] > 0.5 and row["passing_ratio"] > 0.8:
        desc = "Defensa progresivo con balón"
    elif row["fhalf_pressures_ratio"] > 0.5:
        desc = "Defensa de presión alta"
    else:
        desc = "Defensa conservador o de contención"
    return desc

# ==========================================
# PIPELINE POR TIPO DE DEFENSOR
# ==========================================
def process_defenders(df, position_filter, label_suffix):
    ...
    cluster_means["descripcion"] = cluster_means.apply(describe_cluster, axis=1)
    resumen_path = os.path.join(DATA_DIR, f"{TEAM.replace(' ', '_')}_{label_suffix}_summary.csv")
    cluster_means.to_csv(resumen_path, index=False)
    print(f"🧠 Resumen cualitativo guardado en: {resumen_path}\n")
